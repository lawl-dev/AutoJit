...
namespace PerformanceTestConsole
{
    public class AutoJITScriptClass
    {
                ...
                v_Mess =
                    v_Mess.ConcatAssign(
                        _functions.Concat(
                            _functions.Concat(
                                _functions.Concat(
                                    _functions.Concat( _functions.Concat( Variant.Create( "Pass " ), v_i ), Variant.Create( " = " ) ), v_Chrono[v_i] ),
                                Variant.Create( "ms" ) ), _context.CRLF ) );
        ...
        private Variant v_Chrono = Variant.Create( (object) null );
        private Variant v_Mess = Variant.Create( (object) null );
        private Variant v_go = Variant.Create( (object) null );

        public Variant f_getPrimesTo() {
            _functions.DllCall(
                Variant.Create( "kernel32.dll" ), Variant.Create( "INT" ), Variant.Create( "Beep" ), Variant.Create( "DWORD" ), Variant.Create( 1000 ),
        ...
        public Variant f__Report() {
            ...
            v_Mess =
                v_Mess.ConcatAssign(
                    _functions.Concat( _functions.Concat( _functions.Concat( Variant.Create( "Min: " ), f__Minn() ), Variant.Create( "ms" ) ), _context.CRLF ) );
            v_Mess =
                v_Mess.ConcatAssign(
                    _functions.Concat( _functions.Concat( _functions.Concat( Variant.Create( "Max: " ), f__Maxx() ), Variant.Create( "ms" ) ), _context.CRLF ) );
            v_Mess =
                v_Mess.ConcatAssign(
                    _functions.Concat( _functions.Concat( _functions.Concat( Variant.Create( "Ave: " ), f__Ave() ), Variant.Create( "ms" ) ), _context.CRLF ) );
