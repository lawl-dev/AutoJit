...
namespace AutoJIT.CSharpConverter.ConversionModule.ExpressionConverter
{
    internal sealed class AutoitBinaryExpressionConverter : AutoitInvocationExpressionConverterBase<BinaryExpression>
    {
        ...
        private bool NeedsCompilerFunctionCall( Token @operator ) {
            return @operator.Type == TokenType.StringEqual || @operator.Type == TokenType.Pow || @operator.Type == TokenType.AND
                   || @operator.Type == TokenType.OR || @operator.Type == TokenType.Concat;
        }
        ...
        private ExpressionSyntax CreateCompilerFunctionCall( BinaryExpression node, IContextService context ) {
            ...
            IEnumerable<ArgumentSyntax> arguments = Utils.GetEnumerable(
                                                                        SyntaxFactory.Argument( leftExpressionSyntax ),
                                                                        SyntaxFactory.Argument( rightExpressionSyntax ) );
