...
namespace AutoJIT.CSharpConverter.ConversionModule.StatementConverter
{
    ...
    {
        public AutoitRedimStatementConverter( ICSharpStatementFactory cSharpStatementFactory, IInjectionService injectionService )
        ...
        public override IEnumerable<StatementSyntax> Convert( ReDimStatement statement, IContextService context ) {
            ...
            statement.ArrayExpression.AccessParameter.Select( x => new CSharpParameterInfo( Convert( x, context ), false ) );

            toReturn.Add(
                         CSharpStatementFactory.CreateInvocationExpression( variableName, CompilerHelper.GetVariantMemberName( x => x.ReDim() ), parameter )
                                               .ToStatementSyntax() );
