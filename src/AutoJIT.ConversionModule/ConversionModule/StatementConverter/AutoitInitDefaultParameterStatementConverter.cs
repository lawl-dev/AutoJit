...
namespace AutoJIT.CSharpConverter.ConversionModule.StatementConverter
{
    ...
    {
        public AutoitInitDefaultParameterStatementConverter( ICSharpStatementFactory cSharpStatementFactory, IInjectionService injectionService )
        ...
        public override IEnumerable<StatementSyntax> Convert( InitDefaultParameterStatement statement, IContextService context ) {
            ...
            SyntaxFactory.IfStatement(
                                      SyntaxFactory.BinaryExpression(
                                                                     ...
                                                                     SyntaxFactory.IdentifierName( statement.ParameterName ),
                                                                     SyntaxFactory.LiteralExpression( SyntaxKind.NullLiteralExpression ) ),
                                      SyntaxFactory.BinaryExpression(
                                                                     SyntaxKind.SimpleAssignmentExpression,
                                                                     SyntaxFactory.IdentifierName( statement.ParameterName ),
                                                                     Convert( statement.DefaultValue, context ) ).ToStatementSyntax() ).ToEnumerable();
        }
    }
}
